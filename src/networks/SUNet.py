...
class SUNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
                 ...
                 use_checkpoint=True, final_upsample="Dual up-sample", **kwargs):
        super(SUNet, self).__init__()

        # Convert img_size to tuple - handle lists from Hydra config
        ...
        if self.final_upsample == "Dual up-sample":
            # Handle both single int and tuple img_size
            img_size_tuple = to_2tuple(img_size)
            patch_size_tuple = to_2tuple(patch_size)
            self.up = UpSample(input_resolution=(img_size_tuple[0] // patch_size_tuple[0],
...
if __name__ == '__main__':
    from src.utils import network_parameters
